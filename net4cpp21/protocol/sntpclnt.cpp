...
/////////////////////////////////CSNTPClient Implementation //////////////////////////////
CSNTPClient :: CSNTPClient()
{
	m_dwTimeout = SNTP_MAX_RESPTIMEOUT;
	m_hToken=NULL;
}

bool CSNTPClient::GetServerTime(const char * szHostName, NtpServerResponse& response,int nPort)
{
	if(szHostName==NULL || szHostName[0]==0) return false;
	
	socketUdp udp;
	m_strLastError.assign("Failed to create UDP\n");
	if(udp.Open(0)<=0) return false;
	udp.setRemoteInfo(szHostName,nPort);
	...
	if(udp.Send(nSendSize,(const char *)&nbi,-1)<=0)
	{
		m_strLastError.assign("Failed to send NTP request to SNTP server\n");
		return false;
	}
    ...
    NtpFullPacket nfp; //read back the response into the NtpFullPacket struct
    int nReceiveSize = sizeof(NtpFullPacket);
    ZeroMemory(&nfp, nReceiveSize);
	if(udp.Receive((char *)&nfp, nReceiveSize,-1)<=0)
	{
		m_strLastError.assign("Unable to read reply from the SNTP server\n");
		return false;
	}
...
//		return false;
	...
	return SetClientTime(newTime);
}

bool CSNTPClient::EnableSetTimePriviledge()
{
	if(m_hToken) CloseHandle(m_hToken); m_hToken=NULL;
	...
	m_strLastError.assign("Failed to get Adjust priviledge token\n");
	if(m_hToken==NULL) return false;

	ZeroMemory(&m_TokenPriv, sizeof(TOKEN_PRIVILEGES));
	m_strLastError.assign("Failed in callup to lookup priviledge\n");
	if (!LookupPrivilegeValue(NULL, SE_SYSTEMTIME_NAME, &m_TokenPriv.Privileges[0].Luid))
	{ CloseHandle(m_hToken); m_hToken=NULL;	return false; }
	...
	if(AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0))
	  return true;
	m_strLastError.assign("Failed to adjust SetTime priviledge\n");
	return false;	
}

void CSNTPClient::RevertSetTimePriviledge()
{
  if(m_hToken==NULL) return;
  m_TokenPriv.Privileges[0].Attributes &= (~SE_PRIVILEGE_ENABLED);
...
bool CSNTPClient::SetClientTime(const CNtpTime& NewTime)
{
	...
	if (EnableSetTimePriviledge())
	{
		...
		bSuccess = (SetSystemTime(&st))?true:false;
	}
	RevertSetTimePriviledge();
	return bSuccess;
}

/////////////////////////////////CNtpTime Implementation //////////////////////////////
